            ...
            for client in clients:
                if client != client_socket:
                    try:
                        ...
                        # Xử lý client bị ngắt kết nối
                        clients.remove(client)
    except:
        # Xử lý client hiện tại bị ngắt kết nối
        clients.remove(client_socket)
    finally:
        print("Đã ngắt kết nối:", client_socket.getpeername())
        # Đảm bảo client_socket được xóa khỏi danh sách và đóng
        if client_socket in clients: # Kiểm tra lại để tránh lỗi nếu đã bị remove trong except block
            clients.remove(client_socket)
        client_socket.close()
